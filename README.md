# lora
you might ask what is lora?

LoRa modules enable multi-mile wireless communication by using a patented spread spectrum modulation approach developed by Semtech called LoRa. It uses a form of spread spectrum modulation known as Chirp Spread Spectrum (CSS) whose frequency increases or decreases over time across the channel bandwidth 

This LoRa module uses Sub GHz frequencies to transmit and receive messages using sub gigahertzs frequencies of radio waves. below are the compoenents used for this! 

    SX1262IMLTRT for transmission and reception.
    RP2040 for Programing
    0900FM15D0039E as an RF filter.
    AP2112K-3.3 as my LDO.
    PE4259 as my RF switch.
    Two quartz timer crystals (12 MHz and 32 MHz).
    
# PCB pictures

<img width="1221" height="786" alt="image" src="https://github.com/user-attachments/assets/75a21dcd-c694-4b51-a172-9fd8fd26112f" />
<img width="529" height="620" alt="image" src="https://github.com/user-attachments/assets/b9e41589-29e9-4283-8472-f1ebf5f0c82b" />
<img width="724" height="900" alt="image" src="https://github.com/user-attachments/assets/400c9696-9ffd-4103-b6cb-b32afaad44fe" />
